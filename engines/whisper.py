...
class WhisperOpenai:
    ...
    # ------------------------------------------------------------------
    # Streaming path — single-final-at-EOS for the M2 WebSocket server.
    ...
    async def stream(self, pcm_frames, transcription_config):
        ...
        tmp = Path(f"/tmp/whisper_stream_{id(buffer)}.wav")
        tmp.write_bytes(wav_bytes.getvalue())
        try:
            result_dict = await asyncio.to_thread(
                self._transcribe_sync, tmp, model_name, language, None
            )
        finally:
            try:
                tmp.unlink()
        ...
        transcript = (result_dict.get("text") or " ".join(w.content for w in sw)).strip()
        yield StreamFinal(
