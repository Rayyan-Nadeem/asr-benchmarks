...
INTEGRATION_MODES = ("overlay", "segment_first")
...
async def run_one(engine_name: str, case_name: str, fast: bool, transcription_overlay: dict | None, tag: str | None = None, diarizer_name: str = "native", integration_mode: str = "overlay") -> dict[str, Any]:
    ...
    mode_suffix = f"__mode-{integration_mode}" if integration_mode != "overlay" else ""
    run_id = f"{timestamp}__{engine_name}__{case_name}{diar_suffix}{mode_suffix}{tag_suffix}"
    ...
    print(f"      integration: {integration_mode}")
    ...
    use_segment_first = (
        ...
        and diarizer_name not in ("native", "none")
    )
    with ResourceSampler(resources_path, containers=containers, interval_s=1.0) as sampler:
        if use_segment_first:
            # Diarize first, then run engine per segment (boundary-aware integration).
            diar_segments = await diarizer.diarize(case["audio_wav"])
            result = await transcribe_segment_first(
                engine, case["audio_wav"], diar_segments, config=config
            )
        else:
            # Legacy overlay: full-audio ASR, then diarize, then midpoint lookup.
            result = await engine.transcribe(case["audio_wav"], config=config)
            diar_segments = await diarizer.diarize(case["audio_wav"])
    res_summary = sampler.summary(skip_initial_seconds=10.0)

    # In segment_first mode, speaker labels are already assigned per segment.
    # In overlay mode, run the midpoint-lookup overlay now.
    if not use_segment_first and diar_segments:
...
def main() -> None:
    ...
    p.add_argument(
        "--integration-mode",
        choices=list(INTEGRATION_MODES),
        default="overlay",
        help="how engine + diarizer outputs are combined. "
             "'overlay': full-audio ASR, then diarize, then midpoint speaker lookup (default; cheap but loses boundary precision). "
             "'segment_first': diarize first, run ASR per speaker segment, assign segment's speaker to its words (boundary-aware; ~3x slower).",
    )
    args = p.parse_args()
    ...
    if args.concurrency_ramp:
        ...
        asyncio.run(run_one(
            args.engine, args.case, args.fast, overlay or None,
            tag=args.tag,
